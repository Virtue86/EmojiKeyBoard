...
- (void)initStickers
{
    ...
    NSString *path = [emojiBundle pathForResource:@"EmojiPackageList" ofType:@"plist"];
//    NSString *path = [NSBundle.mainBundle pathForResource:@"EmojiPackageList" ofType:@"plist"];
    if (!path) {
        return;
    }
    ...
    for (NSDictionary *stickerDict in array) {
        ...
        NSArray *emojiArr = stickerDict[@"emojis"];
        ...
        for (NSDictionary *emojiDict in emojiArr) {
            ...
            emoji.folderName = stickerDict[@"folderName"];
...
//            emojiSubString = [emojiSubString substringWithRange:NSMakeRange(0, emojiSubString.length - 1)];    // 去掉]
            ...
            if (emoji) {
                ...
                emojiMatchingResult.emojiImage = [UIImage imageNamed:[@"EmojiPackage.bundle" stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/%@",emoji.folderName, emoji.imageName]]];
