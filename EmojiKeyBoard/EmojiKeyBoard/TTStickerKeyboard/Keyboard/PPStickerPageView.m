...
- (void)configureWithSticker:(PPSticker *)sticker
{
    ...
    for (PPEmoji *emoji in emojis) {
        ...
        [button setImage:[self emojiImageWithEmoji:emoji] forState:UIControlStateNormal];
        index += 1;
    }
...
- (UIImage *)emojiImageWithEmoji:(PPEmoji *)emoji
{
    if (!emoji) {
        return nil;
    }
    
    UIImage *image = [UIImage imageNamed:[@"EmojiPackage.bundle" stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/%@", emoji.folderName, emoji.imageName]]];
    return image;
}
